...
import { generateStudyPlan, generateStudyPlanSchema } from '@/domains/study-plan';
...
export async function POST(request: NextRequest, context: RouteContext): Promise<Response> {
  try {
    ...
    const payload = generateStudyPlanSchema.parse(body);
    const plan = await generateStudyPlan({ workspaceId, userId: user.uid, payload });
