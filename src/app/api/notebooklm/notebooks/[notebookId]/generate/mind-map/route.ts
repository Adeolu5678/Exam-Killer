...
export const POST = withAuth(async (req: NextRequest, { userId }) => {
  try {
    ...
    const { error } = await parseBodyWithZod(req, MindMapSchema);
    ...
    const content = await client.generate(notebook.profile_name, notebookId, 'mind-map');

    await incrementUsage(notebook.profile_name, 'query');

    return successResponse({ content });
  } catch (e: unknown) {
    console.error('NLM Mind Map Error:', e);
